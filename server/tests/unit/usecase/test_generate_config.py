...
def test_generate_config_total_cost_lower_than_budget():
    ...
    # Create test data
    affordable_cpu = Cpu(name="Test CPU", price=1000, score=85, ddr=4, socket="AM4", apu_score=30)
    ...
    gpu_repo_mock.get_all_by_budget.return_value = []
    
    sut = GenerateConfigUseCase(cpu_repo_mock, mobo_repo_mock, ram_repo_mock, gpu_repo_mock)
    budget = 5000
...
def test_generate_config_get_best_processing_performance_on_budget():
    # Create mock repositories
    cpu_repo_mock = Mock()
    mobo_repo_mock = Mock()
    ram_repo_mock = Mock()
    ...
    # Create test data
    affordable_weak_cpu = Cpu(name="Test weak CPU", price=1000, score=85, ddr=4, socket="AM4", apu_score=20)
    affordable_strong_cpu = Cpu(name="Test strong CPU", price=1000, score=92, ddr=4, socket="AM4", apu_score=30)
    expensive_cpu = Cpu(name="Expensive CPU", price=6000, score=95, ddr=4, socket="AM4")
    test_mobo = MotherboardCategory(average_price=200, socket="AM4", ddr=4)
    test_ram = RamCategory(average_price=150, ddr=4, frequency=3200, gb_capacity=16)
...
def test_generate_config_get_best_overall_performance_on_budget():
    # Create mock repositories
    cpu_repo_mock = Mock()
    mobo_repo_mock = Mock()
    ram_repo_mock = Mock()
    ...
    cheap_gpu = Gpu(name="Test GPU", price=1000, score=85, ddr=4, socket="AM4")
    expensive_gpu = Gpu(name="Test GPU", price=4500, score=100, ddr=4, socket="AM4")
    
    # Configure mock return values
    cpu_repo_mock.get_all_by_budget.return_value = [expensive_cpu, affordable_weak_cpu,  affordable_strong_cpu]
    mobo_repo_mock.get_all_by_budget.return_value = [test_mobo]
    ram_repo_mock.get_all_by_budget.return_value = [test_ram]
    gpu_repo_mock.get_all_by_budget.return_value = [cheap_gpu, expensive_gpu]
